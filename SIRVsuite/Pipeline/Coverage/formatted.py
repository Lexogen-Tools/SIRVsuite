...
class SIRVsuite_coverage:
    ...
    def tagsExtract(self, featureNames, line):
        ...
        main_colNames = ["seqname", "source", "feature",
    ...
    def readGtf(self, fileName):
        with open(fileName, 'r') as file_object:
            requiredTags = ["seqname", "feature", "start",
            ...
            for line in file_object:
                ...
                if (tags[1] != "exon"):
                    continue
                df = df.append(
                    pd.Series(tags, index=df.columns),
                    ignore_index=True)
            try:
    ...
    def reduceGTF(self):
        ...
        os.system(
            'grep "' + command + '" ' + self.annotationFile + ' > ' + self.outPath +
            '"temp/reduced.gtf"')
        ...
        if (fileType == "gtf"):
            with open(annotationFile, 'r') as file_object:
                ...
                for line in file_object:
                    ...
                    if (tags[1] != "exon"):
                        continue
                    extracted_DF = extracted_DF.append(
                        pd.Series(tags, index=extracted_DF.columns),
                        ignore_index=True)
                try:
            ...
            mixes = pd.read_csv(self.mixPath, header=None,
    ...
    def annot2UTR(self, annotation_df):
        ...
        # Creating an empty data frame with the same structure as annotation data frame
        UTR_annotation = annotation_df[0:0]

        for strand in self.strands:
            transcripts = np.unique(
                                    annotation_df
            ...
            for transcript in transcripts:
                ...
                if (strand == '+'):
                    annot_transcript = annot_transcript.sort_values(
                        by=["start"],
                        axis=0, ascending=False).reset_index(
                        drop=True)
                    ...
                    for index, length in enumerate(exon_lengths):
                        ...
                        if (remainder < 0):
                            UTR_data_frame.loc[index,
                                "start"] = UTR_data_frame.loc[index, "start"] - remainder
                    ...
                    annot_transcript = annot_transcript.sort_values(
                        by=["start"],
                        axis=0, ascending=True).reset_index(
                        drop=True)
                    exon_lengths = annot_transcript["end"] - annot_transcript["start"]
                    ...
                    for index, length in enumerate(exon_lengths):
                        ...
                        if (remainder < 0):
                            UTR_data_frame.loc[index,
                                "end"] = UTR_data_frame.loc[index, "end"] + remainder
                            UTR_data_frame = UTR_data_frame.loc[0:index, :]
                            break
    ...
    def process_annotation(self):
        ...
        if (self.fastaPath != None):
            if (not os.path.isfile(self.fastaPath)):
                ...
                self.fastaData = self.readAnotationFile(
                    self.fastaPath, fileType)
        ...
        if (self.mixPath != None):
            ...
            print(
                "Mixfile not provided.. transcripts for visualization taken from annotation..")
        ...
        if (not os.path.isfile(self.annotationFile)):
            ...
            if (not self.allGenes):
                self.reduceGTF()
                dataFrame = self.readAnotationFile(
                    self.outPath+"temp/reduced.gtf", fileType)
                self.genes = np.unique(dataFrame["gene_id"])
                self.deleteTemporaryFile()
            else:
                dataFrame = self.readAnotationFile(
                    self.annotationFile, fileType)

            for index in np.arange(0, len(self.genes)):

                if (self.annotationFile[-3:] == "gtf"):

                    selectedDF = dataFrame[(
                        dataFrame["gene_id"] == self.genes[index])]

                    ## LIMITING GAPDH TO THE MOST DOMINANT TRANSCRIPT ##
                    if (self.genes[index] == "ENSG00000111640"):
                        selectedDF = dataFrame[(
                            dataFrame["transcript_id"] == "ENST00000229239")]
                    ...
                    if (np.shape(selectedDF)[0] == 0):
                        ...
                        if (self.UTRmode):
                            dataFrames["UTR_only"][self.genes[index]
                                ] = self.annot2UTR(selectedDF)
    ...
    def calculate_real_coverage_BigWig(self, bigwig_path):
        ...
        for file_index, bigwig_path in enumerate(bigwig_path):
            ...
            for gene in self.genes:
                ...
                chromosomeNames = np.unique(
                    gene_annotation["seqname"]).astype(str)
                ...
                for strand in self.strands:
                    ...
                    realCov[sampleName][gene][strand] = file.values(
                        chromosomeNames[0], start, end)
        ...
        if (self.average):
            ...
            for gene in self.genes:
                ...
                for strand in self.strands:
                    ...
                    realCov["avg"][gene][strand] = np.zeros(
                        len(realCov["genePositions"][gene]))
    ...
    def calculate_real_coverage(self, bamFileList):
        ...
        for file_index, bamPath in enumerate(bamList):
            ...
            if ((not os.path.isfile(bamPath)) | (not bamPath[-4:] == ".bam")):
                print(
                    "Provided path for .bam file is not correct... Please try again...")
                return
            ...
            for gene in self.genes:
                ...
                ##-------------------------##
                for fragmentRead in bamFile.fetch(
                    str(chromosomeNames[0]),
                    start, end):
                    ...
                    relative_indexes = fragmentRead.positions - \
                        genePositions[0]

                    last_pos = genePositions[-1] - genePositions[0]
                    ids = np.where(
                        np.logical_and(
                            relative_indexes > 0, relative_indexes <= last_pos))
                ...
                if (self.createCovFile):
                    for strand in self.strands:
                        ...
                        file = open(
                            "".join(
                                [outPath, "gene_coverage_", strandName,
                                 ".cov"]),
                            "w", encoding="utf-8")

                        for i in np.arange(
                            0, len(realCov[sampleName][gene][strand])):
                            file.write("".join([gene, "\t", str(genePositions[i]), "\t", str(
                                int(realCov[sampleName][gene][strand][i])), "\n"]))
    ...
    def calculate_theoretical_coverage(self):
        ...
        for mode in list(self.selectedAnnotations):
            ...
            for gene in self.genes:
                ...
                maxIndex = np.max(
                    np.array(self.selectedAnnotations["whole"][gene]["end"]))
                minIndex = np.min(
                    np.array(
                        self.selectedAnnotations["whole"][gene]["start"]))
                ...
                usedTranscriptsAll = np.unique(
                    np.array(mix_DF[1][mix_DF[0] == gene]).flatten())

                if (len(usedTranscriptsAll) == 0):
                    if (self.mixPath != None):
                        print(
                            "gene " + gene +
                            " not found in mix file.. considering molarity 1 for all transcripts in gtf file..")

                    defaultMolarity = True
                    usedTranscriptsAll = np.unique(
                        gene_annotation["transcript_id"]).flatten()

                theoretical_coverage[mode][gene]["statistics"][
                    "allUsedTranscriptsCount"] = len(usedTranscriptsAll)
                ...
                for direction in self.strands:  # looping over strands
                    ...
                    usedTranscripts = sorted(
                        gene_annotation["transcript_id"]
                        [(gene_annotation["strand"] == direction) &
                         (gene_annotation["transcript_id"].isin(
                              usedTranscriptsAll))].unique())

                    inc = 0
                    if (len(usedTranscripts) > 0):

                        theoretical_coverage[mode][gene][direction][
                            "transcripts"]["names"] = usedTranscripts
                        theoretical_coverage[mode][gene][direction]["transcripts"]["matrix"] = np.zeros(
                            (len(usedTranscripts), len(sumCoverages)))
                        ...
                        for usedTranscript in usedTranscripts:
                            ...
                            if (defaultMolarity):
                                ...
                                selected_mix_row = np.array(
                                    mix_DF[2][mix_DF[1] == usedTranscript]).flatten()

                            if (len(selected_mix_row) > 1):
                                print(
                                    "Multiple rows for transcript " +
                                    usedTranscript +
                                    " in mix file!! Taken first row value..")

                            starts = np.array(gene_annotation["start"][(gene_annotation["transcript_id"] == usedTranscript) & (
                                gene_annotation["strand"] == direction)]).flatten()
                            starts -= minIndex
                            ends = np.array(gene_annotation["end"][(gene_annotation["transcript_id"] == usedTranscript) & (
                                gene_annotation["strand"] == direction)]).flatten()
                            ends -= minIndex + 1
                            molarity = float(selected_mix_row)
                            transcriptIndex = int(
                                usedTranscripts.index(usedTranscript))

                            for i in np.arange(0, len(starts)):
                                theoretical_coverage[mode][gene][direction][
                                    "transcripts"]["matrix"][transcriptIndex,
                                    starts[i]: ends[i]] += molarity  # increasing coverage weighted by the molarity

                                if ((showTransitionLengths) & (i == 0)):
                                    exonTotalLength = np.sum(ends - starts)
                                    startDistance, _, _ = self.calc_start_transition(
                                        exonTotalLength, 30, 25)
                                    startDistance = np.flip(startDistance, axis=0)[
                                                            0:(ends[i]-starts[i])]
                                    theoretical_coverage[mode][gene][direction]["transcripts"][
                                        "matrix"][transcriptIndex, starts[i]:ends[i]] *= startDistance

                                if ((showTransitionLengths) & (i == len(starts)-1)):
                                    exonTotalLength = np.sum(ends - starts)
                                    startDistance, _, _ = self.calc_start_transition(
                                        exonTotalLength, 25, 25)
                                    startDistance = startDistance[len(
                                        startDistance)-(ends[i]-starts[i]):len(startDistance)+1]
                                    theoretical_coverage[mode][gene][direction]["transcripts"][
                                        "matrix"][transcriptIndex, starts[i]:ends[i]] *= startDistance
                        ...
                        theoretical_coverage[mode][gene][direction][
                            "transcripts"]["starts"] = starts
                        theoretical_coverage[mode][gene][direction][
                            "transcripts"]["ends"] = ends
                        theoretical_coverage[mode][gene][direction]["coverage"] = np.sum(
                            theoretical_coverage[mode][gene][direction]["transcripts"]["matrix"], axis=0)

                        sumCoverages += theoretical_coverage[mode][gene][
                            direction]["coverage"]

                        maxMolarity[strand_index1] = np.max(
                            theoretical_coverage[mode][gene][direction]
                            ["transcripts"]["matrix"])
                        maxCoverage[strand_index1] = np.max(
                ...
                theoretical_coverage[mode][gene]["statistics"][
                    "maxMolarity"] = np.max(maxMolarity)
                theoretical_coverage[mode][gene]["statistics"][
                    "maxCoverage"] = np.max(maxCoverage)
            ...
            for gene in self.genes:
                ...
                for sampleName in self.sampleNames:
                    ...
                    for strand in self.strands:
                        ...
                        coverage = self.theoretical_coverage[mode][gene][
                            strand]["coverage"]
                        real_coverage = self.realCoverage[sampleName][gene][
                            strand]
                        ...
                        if (len(coverage) != 0):
                            ...
                            if (mode == "UTR_only"):
                                real_coverage = np.array(
                                    coverage > 0).astype(float) * real_coverage

                            COD, scaling_coefficient = self.CoD(
    ...
    def CoD(self, realCov, theoryCov):
        ...
        # sumBAC (sum of base annotation count), sum of expected coverage
        sumBAC_theory = np.sum(theoryCov)

        sumCoverage_real = np.sum(realCov)  # sum of real coverage

        # calculation of scaling factor for real coverage (for comparison)
        scaling_coefficient = sumCoverage_real/sumBAC_theory
    ...
    def calc_start_transition(self, transcriptLength, mean=200, std=80):
        ...
        lengthProbability = norm.pdf(
            np.arange(1, transcriptLength+1), mean, std)
        ...
        for length in np.arange(1, transcriptLength):
            fragmentProbability[length, 0:transcriptLength-length +
                1] = lengthProbability[length]/(transcriptLength-length+1)

        startDistance = np.sum(fragmentProbability, axis=0)

        adjustedLength = np.sum(
            lengthProbability *
            (transcriptLength - np.arange(1, transcriptLength + 1) + 1))
    ...
    def draw_text(
            self, text=None, x=None, y=None, font_size=28, rotation=0,
            color_rgb=(0, 0, 0),
            ...
            for formatIndex in np.arange(0, len(formats)):
                ...
                if ( formats[formatIndex] == "svg" ):
                    surfaces.append(
                        cairo.SVGSurface(
                            "".join(
                                [outPath, "coverage_", NAME, "_", gene,
                                 ".svg"]),
                            width_paper, height_paper))
                    ...
                    surfaces.append(
                        cairo.ImageSurface(
                            cairo.FORMAT_ARGB32, width_paper, height_paper))
                ...
                modes = np.flip(
                    np.sort(list(self.selectedAnnotations)),
                    axis=0)
                ...
                for mode in modes:
                    ...
                    coverage_max = theoretical_coverage[mode][gene][
                        "statistics"]["maxCoverage"]
                    ...
                    allTranscriptsCount = theoretical_coverage[mode][gene][
                        "statistics"]["allUsedTranscriptsCount"]
                    maxMolarity = theoretical_coverage[mode][gene][
                        "statistics"]["maxMolarity"]

                    StatisticsY = coveragePlot2_Y - coveragePlot2_height / 2
                    StatisticsX = coveragePlot_offsetX / 4

                    segmentStarts = theoretical_coverage["whole"][gene][
                        "segments"]["starts"]
                    segmentEnds = theoretical_coverage["whole"][gene][
                        "segments"]["ends"]
                    ...
                    for strand in ["-", "+"]:
                        ...
                        matrix2draw = theoretical_coverage[mode][gene][
                            strand]["transcripts"]["matrix"]

                        if (len(matrix2draw) == 0):
                            continue

                        sirvTranscripts = theoretical_coverage[mode][gene][
                            strand]["transcripts"]["names"]
                        starts = theoretical_coverage["whole"][gene][strand]["transcripts"]["starts"]
                        ends = theoretical_coverage["whole"][gene][strand][
                            "transcripts"]["ends"]
                        ...
                        for i in np.arange(0, len(sirvTranscripts)):
                           ...
                           cr.set_font_size(exonHeight*12/10)
                            ...
                            segmentExonsLength_draw = (
                                lineLength)-(segmentCount+1)*segmentGaps
                            ...
                            for j in np.arange(0, segmentCount):
                                ...
                                basic_scaling = (
                                    coveragePlot2_height)/(coverage_max*exonHeight)

                                exonStarts, exonEnds = self.returnChanges_greater0(
                                    exonSegment)

                                segmentLength = exonLengths[j]/sumOfLengths*(
                                    segmentExonsLength_draw)
                                ...
                                if (len(exonStarts) > 0 ):
                                    ...
                                    for exonIndex in range(0, len(exonStarts)):
                                        exonStartPosX = startSegmentsDraw+ \
                                            exonStarts[exonIndex]/len(exonSegment)*segmentLength
                                        exonLength = startSegmentsDraw+exonEnds[exonIndex]/len(
                                            exonSegment)*segmentLength-exonStartPosX
                                ...
                                if (( self.UTRmode and mode == "UTR_only" ) or ( self.UTRmode == False and mode == "whole" ) ):
                                    ...
                                    if (i == 0 ):
                                        ...
                                        if strand == noNullStrand:
                                            ...
                                            x_axis_coords = str(
                                                self.realCoverage["genePositions"][gene][segmentStarts[j]])
                                            ...
                                            x_axis_coords = str(
                                                self.realCoverage["genePositions"][gene][segmentEnds[j]])
                                            (_, _, _, height_text, _, _) = cr.text_extents(x_axis_coords)
                                        ...
                                        coverageSegment = theoretical_coverage[mode][gene][
                                            strand]["coverage"][segmentIndexStart:segmentIndexEnd]
                                        ...
                                        for sampleName in sampleNames:
                                            ...
                                            segment_start = theoretical_coverage[
                                                "whole"][gene]["segments"]["starts"][j]
                                            segment_end = theoretical_coverage["whole"][gene]["segments"]["ends"][j]

                                            realCoverageSegment = self.realCoverage[sampleName][
                                                gene][strand][segment_start:segment_end]
                                            ...
                                            if ('NaN' not in scaling_coefficients.values()):
                                                coverage_scaling_coef = max(
                                                    list(scaling_coefficients.values()))
                                            ...
                                            if (scaling_coefficients["+"] == "NaN"):
                                                ...
                                                max_visible_read["+"] = abs(
                                                    coveragePlot2_Y - limit_draw_upper) * coverage_scaling_coef * relative_scaling / basic_scaling / exonHeight

                                            if (scaling_coefficients["-"] == "NaN"):
                                                max_visible_read["-"] = None
                                            else:
                                                max_visible_read["-"] = abs(
                                                    coveragePlot2_Y - limit_draw_lower) * coverage_scaling_coef * relative_scaling / basic_scaling / exonHeight
                                            ...
                                            for realCovarageIndex in np.arange(0, len(realCoverageSegment)):
                                                if (strand == "-" ):
                                                    value = - \
                                                        realCoverageSegment[realCovarageIndex]
                                                ...
                                                draw_X = startSegmentsDraw + realCovarageIndex/ \
                                                    len(realCoverageSegment)*segmentLength
                                            ...
                                            start_exceed, end_exceed = self.returnChanges_greater0(
                                                exceedLimitSegment)
                                            ...
                                            if (len(start_exceed) > 0):
                                                ...
                                                start_exceed_mark = list(
                                                    start_exceed)
                                                end_exceed_mark = list(
                                                    end_exceed)
                                                removed = 0
                                                ...
                                                for index in range(
                                                    len(start_exceed)):
                                                    start = startSegmentsDraw + start_exceed[index] / len(
                                                        realCoverageSegment) * segmentLength
                                                    end = startSegmentsDraw + end_exceed[index] / len(
                                                        realCoverageSegment) * segmentLength
                                                ...
                                                for index in range(
                                                    len(start_exceed_mark)):
                                                    start = startSegmentsDraw + start_exceed_mark[index] / len(
                                                        realCoverageSegment) * segmentLength
                                                    end = startSegmentsDraw + end_exceed_mark[index] / len(
                                                        realCoverageSegment) * segmentLength
                                                    middle = (end + start ) / 2
                                        ...
                                        for k in np.arange(0, len(indexes4change)-1):
                                            ...
                                            draw_startX = startSegmentsDraw+indexes4change[k]/len(
                                                coverageSegment)*segmentLength
                                            draw_startX_next = startSegmentsDraw+ \
                                                indexes4change[k+1]/len(coverageSegment)*segmentLength
                ...
                if (max_visible_read["+"] is not None):
                    ...
                    cr.move_to(startX_line - 10 + 4,
                               limit_draw_upper + offset_y_arrow)
                    cr.line_to(startX_line - 10, limit_draw_upper)
                    cr.line_to(startX_line - 10 - 4,
                               limit_draw_upper + offset_y_arrow)
                    ...
                    y_label_position_plus = coveragePlot2_Y - (
                        coveragePlot2_Y - limit_draw_upper)/2 - width_text/2
                ...
                if (max_visible_read["-"] is not None):
                    ...
                    cr.move_to(startX_line - 10 + 4,
                               limit_draw_lower - offset_y_arrow)
                    cr.line_to(startX_line - 10, limit_draw_lower)
                    cr.line_to(startX_line - 10 - 4,
                               limit_draw_lower - offset_y_arrow)
                    ...
                    y_label_position_minus = (
                        limit_draw_lower - coveragePlot2_Y)/2 + coveragePlot2_Y - width_text/2
                ...
                for sampleName in self.sampleNames:
                    for strand in ["+", "-"]:
                        COD = self.stats["COD"] [gene] [
                            self.sampleNames[0]] [strand]
                        if (COD is not "NaN" ):
                            COD = str(COD)
