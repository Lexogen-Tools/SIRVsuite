...
The spike-in coverage generated by the SIRVSuite can also be inspected interactively, by loading the measured coverage (stored in <a href=http://genome.ucsc.edu/goldenPath/help/bigWig.html>bigwig</a> (.bw) format) in a genome browser (e.g. the <a href=http://software.broadinstitute.org/software/igv/UserGuide>IGV</a> browser).

Please note that even in this example, the CoD values for the sense and anti-sense coverage each are not exactly
close to 0. Ideally, the coverage over the SIRV genes is primarily determined by the composition of the SIRV 
transcripts, but in real experiments it is unlikely that the reads perfectly cover the transcripts as described 
in the theoretical shape, given that the distribution of reads over a transcript is a product of the library preparation
method and likely subject to a range of biases and in part affected by the read-depth of the samples the NGS experiment.

We prepared a set of simplified example coverages to show how the CoD values are affected by 
differences between the theoretical coverage and the measured coverage in 
[examples/CoD/CoD_examples.xlsx](examples/CoD/CoD_examples.xlsx). The following figure is an excerpt of this file
and displays a CoD value with almost neglibale differences between the theoretical coverage and the measured coverage:

<p align="center"><img src="./examples/CoD/CoD_small.png"></p>

Compared to this, a coverage that is significantly different in shape like in the following picture results in a way 
higher CoD value:
<p align="center"><img src="./examples/CoD/CoD_high.png"></p>

The full set of examples is available in [examples/CoD/CoD_examples.xlsx](examples/CoD/CoD_examples.xlsx).
